...
public class ChordProtocol implements EDProtocol {
	...
	public void stabilize(Node myNode) { 
		try { 
			...
			if (node != null) { 
				...
				if (idInab(chordId,remoteID,((ChordProtocol) successorList[0] .getProtocol(p.pid)).chordId)) successorList[0] = node; 
	...
	private Node closest_preceding_node(BigInteger id) {
		for (int i = m; i > 0; i--) {
			try {
				if (fingerTable[i - 1] == null || fingerTable[i - 1].isUp() == false) {
					continue;
				}
